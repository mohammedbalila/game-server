...
function setSplashScreen() {
  ...
  GAME.splashScreenText = GAME.add.text(
    GAME.world.centerX,
    GAME.world.centerY,
    text,
    GAME_FONT_STYLE,
...
function sendPlayerInfo(self, playerInfo) {}

function create() {
  LOG.info('create');

  this.socket.emit('joinRoom', { player: this.socket.id, room, username });
...
function checkCollision() {
  ...
  if (GAME.player.body.bottom >= GAME.world.bounds.bottom) {
    GAME.gameOverScreen();
  }
...
function welcomeScreen() {
  // set bg
  GAME.background.autoScroll(-SPEED * 0.8, 0);
...
function start() {
  ...
  // spawn wall
  GAME.wallTimer = GAME.game.time.events.loop(
...
function gameOverScreen() {
  ...
  this.socket.emit('gameOver', {
    player: { id: this.socket.id, room, score: GAME.score },
  });
  // set text
...
function spawnWalls() {
  var wallY = GAME.rnd.integerInRange(GAME_HEIGHT * 0.3, GAME_HEIGHT * 0.7);
...
function updateScore(wall) {
  ...
  wall.scored = true;
  GAME.score += 0.5; // cause we have 2 walls (2 instances) in a row (top and bottom)
